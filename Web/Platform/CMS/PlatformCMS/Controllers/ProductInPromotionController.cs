    ...
    public class ProductInPromotionController : ControllerBase
    {
        ...
        public ResponseTwoList GetByProductId(int productId)
        {
            ...
            try
            {
                ...
                responseData.ListObj2 = promotioninProduct.Select(x => new { x.Id, PromotionId = x.PromotionId, ProductId = x.ProductId, Name = promotion.ContainsKey(x.PromotionId ?? 0) ? promotion[x.PromotionId.Value].Name : "", LocationId = x.LocationId }).ToList<object>();
            }
            catch (Exception ex)
            {
        ...
        public ResponseData Add([FromBody] ViewModel.ProductInPromotions obj)
        {
            ...
            try
            {
                bool result = productInPromotionBCL.Add(obj.Id, obj.LocationId, obj.Promotions);
                if (result)
                {
