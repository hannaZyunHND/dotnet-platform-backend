new WOW().init();

function sliderPR() {
    var sliderPR = new Swiper('.slide-banner-after-manu .swiper-container', {
        slidesPerView: 3,
        spaceBetween: 1,
        loop: false,
        //autoplay: {
        //    delay: 1000,
        //  },
        navigation: {
            nextEl: '.slide-five-part-two-line .swiper-button-next',
            prevEl: '.slide-five-part-two-line .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 1,
            },
            567: {
                slidesPerView: 1,
            },
            767: {
                slidesPerView: 1,
            },
            991: {
                slidesPerView: 1,
            },
            1023: {
                slidesPerView: 3,
            },
            1365: {
                slidesPerView: 3,
            },
        },
    });
}

function sliderPR6() {
    var sliderPR = new Swiper('.slide-product.slide-6 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: {
            delay: 2500,
        },
        navigation: {
            nextEl: '.slide-product.slide-1 .swiper-button-next',
            prevEl: '.slide-product.slide-1 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },
    });
}

function sliderPR7() {
    var sliderPR = new Swiper('.slide-product.slide-7 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: {
            delay: 2500,
        },
        navigation: {
            nextEl: '.slide-product.slide-1 .swiper-button-next',
            prevEl: '.slide-product.slide-1 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR8() {
    var sliderPR = new Swiper('.slide-product.slide-8 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: {
            delay: 2500,
        },
        navigation: {
            nextEl: '.slide-product.slide-8 .swiper-button-next',
            prevEl: '.slide-product.slide-8 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR9() {
    var sliderPR = new Swiper('.slide-product.slide-9 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: {
            delay: 2500,
        },
        navigation: {
            nextEl: '.slide-product.slide-1 .swiper-button-next',
            prevEl: '.slide-product.slide-1 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR10() {
    var sliderPR = new Swiper('.slide-product.slide-10 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: {
            delay: 2500,
        },
        navigation: {
            nextEl: '.slide-product.slide-1 .swiper-button-next',
            prevEl: '.slide-product.slide-1 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR2() {
    var sliderPR2 = new Swiper('.slide-product.slide-2 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        //autoplay: {
        //    delay: 1000,
        //  },
        navigation: {
            nextEl: '.slide-product.slide-2 .swiper-button-next',
            prevEl: '.slide-product.slide-2 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}


function sliderPR3() {
    var sliderPR3 = new Swiper('.slide-product.slide-3 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        //autoplay: {
        //    delay: 1000,
        //  },
        navigation: {
            nextEl: '.slide-product.slide-3 .swiper-button-next',
            prevEl: '.slide-product.slide-3 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR4() {
    var sliderPR4 = new Swiper('.slide-product.slide-4 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        //autoplay: {
        //    delay: 1000,
        //  },
        navigation: {
            nextEl: '.slide-product.slide-4 .swiper-button-next',
            prevEl: '.slide-product.slide-4 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}

function sliderPR5() {
    var sliderPR5 = new Swiper('.slide-product.slide-5 .swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        //autoplay: {
        //    delay: 1000,
        //  },
        navigation: {
            nextEl: '.slide-product.slide-5 .swiper-button-next',
            prevEl: '.slide-product.slide-5 .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}


function slidercate() {
    var slidercate = new Swiper('.slide-cate-3-item .swiper-container', {
        slidesPerView: 3,
        spaceBetween: 20,
        loop: true,
        autoplay: {
            delay: 2500,
            disableOnInteraction: true,
        },
        breakpoints: {
            320: {
                slidesPerView: 1,
                spaceBetween: 10,
            },
            767: {
                slidesPerView: 2,
            },
            991: {
                slidesPerView: 2,
            },
            1023: {
                slidesPerView: 3,
            },
            1365: {
                slidesPerView: 3,
            },
        },

    });
}
slidercate();
sliderPR6();
sliderPR7();
sliderPR8();
sliderPR9();
sliderPR10();
sliderPR5();
sliderPR4();
sliderPR3();
sliderPR2();
sliderPR();


function topFunction() {
    $('body,html').animate({
        scrollTop: 0 // Scroll to top of body
    }, 500);
}

function expand() {

    $(".content-cate-intro").toggleClass("expand");
    $('body,html').animate({
        scrollTop: $(".content-cate-intro").offset().top // Scroll to top of body
    }, 500);

}

function expandClient() {
    $(".full-client").toggleClass("expand");

}

$(".list-menu-cate .list-group-item:first-child").click(function() {
    $(this).closest(".list-menu-cate").toggleClass("un-expand");
})


$(window).scroll(function() {
    if ($(this).scrollTop() >= 50) { // If page is scrolled more than 50px
        $('.header').addClass("fixed"); // Fade in the arrow
        //   $(".list-menu-cate").addClass("un-expand");

    } else {
        $('.header').removeClass("fixed"); // Else fade out the arrow

    }
});



function slidebrand() {
    var slidebrand = new Swiper('.slide-brand.swiper-container', {
        slidesPerView: 6,
        spaceBetween: -1,
        loop: true,
        autoplay: true,
        breakpoints: {
            320: {
                slidesPerView: 1.5,
            },
            567: {
                slidesPerView: 2,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 4.5,
            },
            1023: {
                slidesPerView: 5,
            },
            1365: {
                slidesPerView: 6,
            },
        },

    });
}
slidebrand();

function spvx() {
    $(".menu-fix .spvx").toggleClass("show");
    //$(document).mouseup(function (e) {
    //    var container = $(".dropdown-cart");
    //    var container_mobile = $('.dropdown-cart-mobile');
    //    var last_seen_container = $('.menu-fix .spvx');

    //    // if the target of the click isn't the container nor a descendant of the container
    //    //if (!container.is(e.target) && container.has(e.target).length === 0) {
    //    //    container.hide();
    //    //}
    //    //if (!container_mobile.is(e.target) && container_mobile.has(e.target).length === 0) {
    //    //    container_mobile.hide();
    //    //}
    //    if (!last_seen_container.is(e.target) && last_seen_container.has(e.target).length === 0) {
    //        alert(1);
    //        last_seen_container.hide();
    //        //console.log(1);
    //    }
    //});
}




$(".select-local .dropdown-item").click(function() {
    var text = $(this).text();
    $(this).closest(".select-local").find(".dropdown-toggle").text(text);
})

$(".list-menu-right .heading").click(function() {
    $(this).closest(".list-menu-right").toggleClass("expand");
})

$(".star-rating i").click(function() {
    $(this).parent().find(".star-rating i").removeClass("checked");
    $(this).addClass("checked");
    $(this).prevAll().addClass("checked");
    $(this).nextAll().removeClass("checked");

});
$(".star-rating-dynamic i").click(function() {
    $(this).parent().find(".star-rating-dynamic i").removeClass("checked");
    $(this).addClass("checked");
    $(this).prevAll().addClass("checked");
    $(this).nextAll().removeClass("checked");
});

function slidethumb1() {
    var galleryThumbs = new Swiper('.slide-product-detail .gallery-thumbs', {
        spaceBetween: 10,
        slidesPerView: 6,
        watchSlidesVisibility: true,
        watchSlidesProgress: true,
        breakpoints: {
            320: {
                slidesPerView: 3,
            },
            567: {
                slidesPerView: 4,
                spaceBetween: 15,
            },
            767: {
                slidesPerView: 5,
                spaceBetween: 15,
            },
            991: {
                slidesPerView: 6,
                spaceBetween: 10,
            },
            1023: {
                slidesPerView: 6,
                spaceBetween: 15,
            },
            1365: {
                slidesPerView: 5,
                spaceBetween: 15,
            },
        }
        
        //on: {
        //    slideChangeTransitionEnd: function () {
        //        $('.swiper-slide').each(function (element) {
        //            $(this).removeClass('swiper-slide-thumb-active');
        //        })
        //        $('.swiper-slide.swiper-slide-active').addClass('swiper-slide-thumb-active');
        //    }
        //}
    });
    var galleryTop = new Swiper('.slide-product-detail .gallery-top', {
        spaceBetween: 10,
        thumbs: {
            swiper: galleryThumbs
        }
        //, navigation: {
        //    nextEl: '.slide-product-detail .swiper-button-next',
        //    prevEl: '.slide-product-detail .swiper-button-prev',
        //},

    });

}

function slidethumb2() {
    var galleryThumbs2 = new Swiper('.slide-product-detail-2 .gallery-thumbs-2', {
        spaceBetween: 10,
        slidesPerView: 8,
        watchSlidesVisibility: true,
        watchSlidesProgress: true,
        breakpoints: {
            320: {
                slidesPerView: 3,
            },
            567: {
                slidesPerView: 4,
                spaceBetween: 15,
            },
            767: {
                slidesPerView: 5,
                spaceBetween: 10,
            },
            991: {
                slidesPerView: 5,
                spaceBetween: 10,
            },
            1023: {
                slidesPerView: 5,
                spaceBetween: 15,
            },
            1365: {
                slidesPerView: 8,
                spaceBetween: 10,
            },
        },
        navigation: {
            nextEl: '.slide-product-detail-2 .swiper-button-next',
            prevEl: '.slide-product-detail-2 .swiper-button-prev',
        },
    });
    var galleryTop2 = new Swiper('.slide-product-detail-2 .gallery-top-2', {
        spaceBetween: 10,
        thumbs: {
            swiper: galleryThumbs2
        }
    });
}



function slideCate() {
    var sliderBlogcate = new Swiper('.slide-blog-cate.swiper-container', {
        slidesPerView: 1,
        spaceBetween: 15,
        loop: true,
        speed: 600,
        autoplay: true,
        navigation: {
            nextEl: '.slide-blog-cate .swiper-button-next',
            prevEl: '.slide-blog-cate .swiper-button-prev',
        },
    });
}
slideCate();
slidethumb1();
slidethumb2();

function expandListStore() {
    $(".list-add-store").toggleClass("expand");
}

$(".tab-flash-sale .link-tab-flash-sale").click(function() {
    $(".tab-flash-sale .link-tab-flash-sale").removeClass("active");
    $(this).addClass("active");
})

function sidevideo() {
    var sidevideo = new Swiper('.slide-video .swiper-container', {
        slidesPerView: 3,
        spaceBetween: 15,
        autoplay: true,
        loop: true,
        breakpoints: {
            320: {
                slidesPerView: 1,
            },
            375: {
                slidesPerView: 1,
            },
            767: {
                slidesPerView: 2,
            },
            991: {
                slidesPerView: 3,
            },
            1023: {
                slidesPerView: 3,
            },
            1365: {
                slidesPerView: 3,
            },
        },
    });
}

sidevideo();


function expandProductDes() {
    $(".product-description").toggleClass("expand");
    $('body,html').animate({
        scrollTop: $(".product-description").offset().top // Scroll to top of body
    }, 500);
    //$('html, body').animate({
    //    scrollTop: $(".product-description").offset().top
    //}, 200);
}

function sliderlocaltop() {
    var sliderPR = new Swiper('.list-location-top .swiper-container', {
        slidesPerView: 12,
        spaceBetween: 10,
        autoplay: {
            delay: 2000
        },
        loop: true,
        navigation: {
            nextEl: '.list-location-top .swiper-button-next',
            prevEl: '.list-location-top .swiper-button-prev',
        },
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 3,
            },
            767: {
                slidesPerView: 4,
            },
            991: {
                slidesPerView: 5,
            },
            1023: {
                slidesPerView: 7,
            },
            1365: {
                slidesPerView: 10,
            },
        },
    });
}
sliderlocaltop();


$(".list-filter-modal .heading").click(function() {
    $(this).closest(".menu.sub").toggleClass("show");
});

$(".btn-x").click(function() {
    $(".btn-x").removeClass("active");
    $(this).toggleClass("active");
});

function slidethumbhome() {
    var galleryThumbs3 = new Swiper('.slide-banner-home .gallery-thumbs-home', {
        spaceBetween: -1,
        slidesPerView: 4,
        autoplay: true,
        watchSlidesVisibility: true,
        watchSlidesProgress: true,
        breakpoints: {
            320: {
                slidesPerView: 1.5,
            },
            567: {
                slidesPerView: 1.8,
            },
            767: {
                slidesPerView: 2.2,
            },
            991: {
                slidesPerView: 3.2,
            },
            1023: {
                slidesPerView: 3.5,
            },
            1365: {
                slidesPerView: 3.8,
            },
        },
    });
    var galleryTop3 = new Swiper('.slide-banner-home .gallery-top-home', {
        spaceBetween: 10,
        autoplay: true,
        thumbs: {
            swiper: galleryThumbs3
        },
        pagination: {
            el: '.swiper-pagination',
            type: 'bullets',
            clickable: true,
        },
        lazy: {
            loadPrevNext: true,
        },

    });
    //galleryTop3.on('slideChange', function () {
    //    console.log($(this))
    //})
}

slidethumbhome();

function slidermenu() {
    var slidermenu = new Swiper('.menu-op .swiper-container', {
        slidesPerView: 5,
        spaceBetween: 20,
        autoplay: true,
        breakpoints: {
            320: {
                slidesPerView: 2,
            },
            567: {
                slidesPerView: 3,
            },
            767: {
                slidesPerView: 3,
            },
            991: {
                slidesPerView: 3,
            },
            1365: {
                slidesPerView: 4,
            },
        },

    });
}
slidermenu();
$(".list-menu-cate .list-group-item:gt(12)").css("display", "none");
$(".list-menu-cate .list-group-item:last-child").css("display", "block").click(function() {
    $(".list-menu-cate .list-group-item:gt(3)").toggleClass("d-block");
});


function timeIcon() {
    $(".btn-local .onhover").addClass("hide");
}

// ====================Scroll to top========================
jQuery(document).ready(function($) {
    var visible = false;
    //Check to see if the window is top if not then display button
    $(window).scroll(function() {
        var scrollTop = $(this).scrollTop();
        if (!visible && scrollTop > 100) {
            $(".scrollToTop").fadeIn();
            visible = true;
        } else if (visible && scrollTop <= 100) {
            $(".scrollToTop").fadeOut();
            visible = false;
        }
    });
    //Click event to scroll to top
    $(".scrollToTop").click(function() {
        $("html, body").animate({
            scrollTop: 0
        }, 800);
        return false;
    });
});

// =================================
$(document).ready(function() {
    $('.giatot-slide').slick({
        autoplay: true,
        autoplaySpeed: 2500,
        arrows: false,
        slidesToShow: 5,
        slidesToScroll: 1,
        responsive: [{
            breakpoint: 1024,
            settings: {
                slidesToShow: 3,
                slidesToScroll: 1
            }
        }, {
            breakpoint: 480,
            settings: {
                slidesToShow: 2,
                slidesToScroll: 1
            }
        }]
    });
    $('.next-3').click(function() {
        $(".giatot-slide").slick('slickNext');
    });
    $('.pre-3').click(function() {
        $(".giatot-slide").slick('slickPrev');
    });


    var number = $('.slide-prod-bep').data('count');
    var number_mobile = $('.slide-prod-bep').data('count');
    if (number >= 6) {
        number = 6;
    } else {
        number = number;
    }
    if (number_mobile >= 4) {
        number_mobile = 4
    }
    else {
        number_mobile = number_mobile;
    }

    $('.slide-prod-bep').slick({
        dots: false,
        infinite: true,
        speed: 300,
        slidesToShow: number,
        slidesToScroll: 2,
        responsive: [
            {
                breakpoint: 1024,
                settings: {
                    slidesToShow: number,
                    slidesToScroll: 2,
                    infinite: true,
                    dots: true
                }
            },
            {
                breakpoint: 600,
                settings: {
                    slidesToShow: number_mobile,
                    slidesToScroll: 1
                }
            },
            {
                breakpoint: 480,
                settings: {
                    slidesToShow: number_mobile,
                    slidesToScroll: 1
                }
            }
            // You can unslick at a given breakpoint now by adding:
            // settings: "unslick"
            // instead of a settings object
        ]
    });
    $('.next-1').click(function () {
        $(".slide-prod-bep").slick('slickNext');
    });
    $('.pre-1').click(function () {
        $(".slide-prod-bep").slick('slickPrev');
    });
});